...
export default function VendorForm({ spotsLeft }: { spotsLeft: number }) {
  ...
  const inp = 'w-full box-border rounded-md border border-gray-300 px-3 py-2.5 text-sm'
  const lbl = 'mb-1 block text-[13px] font-semibold text-gray-700'
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2">
        <div>
          ...
          <input className={inp} value={form.fullName} onChange={e => set('fullName', e.target.value)} placeholder="Your full name" required />
        </div>
        <div>
          <label className={lbl}>Email Address *</label>
          <input className={inp} type="email" value={form.email} onChange={e => set('email', e.target.value)} placeholder="your@email.com" required />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2">
        <div>
          ...
          <input className={inp} value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="+237 6XX XXX XXX" required />
        </div>
        <div>
      ...
      <div>
        <label className={lbl}>Business / Brand Name *</label>
        <input className={inp} value={form.businessName} onChange={e => set('businessName', e.target.value)} placeholder="Name of your business or brand" required />
      </div>
      ...
      <div>
        <label className={lbl}>Tell us about your business</label>
        <textarea
          className={`${inp} min-h-[90px] resize-y font-[inherit]`}
      ...
      {error && (
        <div className="rounded-md border border-red-200 px-3.5 py-2.5" style={{ background: '#fef2f2' }}>
          <p className="m-0 text-[13px]" style={{ color: '#b91c1c' }}>{error}</p>
        </div>
      )}
      ...
      <button
        type="submit"
        disabled={submitting}
        className="rounded-lg border-none px-6 py-3.5 text-[15px] font-bold text-white"
        style={{
